...
Apply functional programming concepts. A function is something that takes an input, does some work, and may return some output. We build our application this way, without object oriented concepts which can muddy logic and obscure error prone patterns. All functions should be pure, without side effects. Functions should not mutate the data they ingest. They should never affect the execution of code that does not directly depend on their results. No render function should transform its props data into anything but render or assert any business logic. Any data transformation should be done in formatters at the service level. Each container's mapStateToProps pulls the route reducer's state off the rootState and maps it into props. mapDispatchToProps glues actions to event listeners and then maps them into props. Render functions simply display those props. Adhering to this means that bugs are easily identified. If display is incorrect or an event listener does not fire it is in render. Otherwise, it is in business logic.

App design should be device first. This means that you should provide a different version of an experience for a different device. To support implementing this approach, there is a reducer on the root that listens to viewport state. This action is fired in AsyncWrapper's componentDidMount(), which also includes a function to reset window scroll state. Since we use this component to wrap route containers, its componentDidMount runs every route mount.
